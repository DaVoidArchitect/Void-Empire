...
def main():
    ...
    def test_polymorphic_agent():
        with open("../voidos/agent.logos", "r", encoding="utf-8") as f:
            code = f.read()
        smir = compile_logos(code, source_path="../voidos/agent.logos")
        
        # Verify the structure of compiled SMIR
        assert smir.get("logos_version") == "2.0-declarative", "Version mismatch"
        assert len(smir.get("intents", [])) >= 1, "Expected at least 1 intent"
        intent = next(i for i in smir["intents"] if i["name"] == "UniversalAgent")
