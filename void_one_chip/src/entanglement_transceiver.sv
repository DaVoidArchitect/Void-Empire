...
// Void One - Clockless Entanglement Transceiver

`timescale 1ns/1ps

module entanglement_transceiver #(
    parameter int unsigned DATA_W = 32,
    parameter int unsigned PHASE_COUNT = 9
) (
    input  logic                    activation_i,
    input  logic                    state_reset_i,
    input  logic                    transceive_pulse_i,
    ...
    input  logic [PHASE_COUNT-1:0]  phase_vector_i,
    input  logic [DATA_W-1:0]       payload_i,
    output logic [DATA_W-1:0]       entangled_payload_o,
    output logic                    coherence_link_o,
    output logic [15:0]             entanglement_energy_o,
  ...
  logic [DATA_W-1:0] transceiver_state_q;
  logic [15:0]       coherence_seed_q;

  function automatic logic [DATA_W-1:0] phase_scrambler(
      input logic [PHASE_COUNT-1:0] phase_vector,
    ...
    begin
      ...
      for (idx = 0; idx < DATA_W; idx++) begin
        mask[idx] = phase_vector[idx % PHASE_COUNT]
          ^ chakra_band[idx % 3]
          ^ cultivation_stage[idx % 4];
      end
      phase_scrambler = mask;
    end
  endfunction

  // Clockless state updates happen only on explicit transceive pulses.
  always_latch begin
    if (state_reset_i) begin
      transceiver_state_q <= '0;
      coherence_seed_q    <= 16'd3;
    end else if (activation_i && transceive_pulse_i) begin
      if (!collapse_guard_o) begin
        transceiver_state_q <= payload_i
          ^ phase_scrambler(phase_vector_i, chakra_band_i, cultivation_stage_i)
          ^ {DATA_W{defect_density_i[2]}};
      end
      coherence_seed_q    <= 16'(3 + $countones(phase_vector_i) + chakra_band_i + cultivation_stage_i);
    end
  end
  ...
  always_comb begin
    ...
    coherence_link_o      = activation_i && (coherence_seed_q >= 16'd6) && !collapse_guard_o;
    entangled_payload_o   = coherence_link_o ? transceiver_state_q : payload_i;
    entanglement_energy_o = coherence_link_o
      ? (16'(coherence_seed_q + chakra_band_i + cultivation_stage_i + (defect_density_i / 4)))
      : 16'd0;

    entropy_gain_ppm_o = activation_i
      ? (16'd1000 + ($countones(phase_vector_i) * 16'd17)
        + (defect_density_i * 16'd9)
        + (thermal_zone_o == THERMAL_SOVEREIGN ? 16'd190 : 16'd0))
      : 16'd0;
