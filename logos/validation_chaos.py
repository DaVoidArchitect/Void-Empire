...
def run_test_suite():
    ...
    def parse_empty_intent_bodies():
        ...
        assert len([i for i in prog.items if type(i).__name__ == 'IntentNode']) == 1
    ...
    def parse_state_no_transitions():
        ...
        assert len([i for i in prog.items if type(i).__name__ == 'IntentNode'][0].states) == 1
    ...
    def parse_intent_no_states():
        ...
        prog = Parser(tokenize(code)).parse()
        assert len([i for i in prog.items if type(i).__name__ == 'IntentNode'][0].states) == 0
    ...
    def compile_diamond_imports():
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert len([i for i in merged.items if type(i).__name__ == 'IntentNode']) >= 4
    ...
    def vm_resource_exhaustion():
        ...
        code = "intent T { state Init { on cycle_evt -> Init { require { energy: 10.0 J; } }; } }"
        smir = Compiler(mesh).compile(Parser(tokenize(code)).parse())
        vm = LogosVM(smir, mesh, {'t_state': 'Init'})
        for i in range(5):
            res = vm.send_event("T", "cycle_evt")
            assert res['status'] == 'transitioned'
        try:
            vm.send_event("T", "cycle_evt")
    ...
    def integration_logosc_nonexistent_file():
        res = subprocess.run(["uv", "run", "python", "logosc.py", "non_existent.logos"], capture_output=True, text=True)
    ...
    def integration_logosc_binary_file():
        res = subprocess.run(["uv", "run", "python", "logos/compiler.py", "logos/bin/logosc.exe"], capture_output=True, text=True)
        assert res.returncode != 0
    test_case("Integration", "logosc_binary_file", integration_logosc_binary_file)

    def integration_logos_vm_nonexistent_file():
        res = subprocess.run(["uv", "run", "python", "interpreter.py", "non_existent.smir.json"], capture_output=True, text=True)
        assert res.returncode == 1
        assert "no such file" in res.stderr.lower() or "no such file" in res.stdout.lower() or res.returncode != 0
