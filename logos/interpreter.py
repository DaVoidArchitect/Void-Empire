...
class LogosVM:
    ...
    def __init__(
        ...
        import json
        if isinstance(smir, str):
            try:
                self.smir = json.loads(smir)
            except json.JSONDecodeError as e:
                raise LogosRuntimeError(f"Corrupted SMIR string: {e}")
        else:
            self.smir = smir
        self.mesh = dict(mesh_context)  # working copy — will be mutated
        self.runtime_ctx = runtime_context or runtime_ctx or {}
    ...
    def send_event(self, intent_name: str, event: str) -> dict:
        ...
        for trans in state_dict.get("transitions", []):
            ...
            if requires:
                self._atomic_deduct(requires, trans.get("line", -1), event, current_state_name)
            ...
            if constraints:
                ...
                if violation:
                    ...
                    for req in requires:
                        ...
                        mult = 1.0618 if res_name in ("energy", "cycle") else 1.0
                        self.mesh[res_name] = self.mesh.get(res_name, 0.0) + req["value"] * mult
    ...
    def _atomic_deduct(self, requires: list[dict], line: int, event: str = "", state: str = ""):
        """
        Check *all* requirements against the mesh in a read-only pass.
        If all pass, apply deductions atomically.
        If any fail, raise TransitionFrozenError.
        """
        # Phase 1: Read-only verification
        for req in requires:
            resource = req["resource"]
            mult = 1.0618 if resource in ("energy", "cycle") else 1.0
            needed = req["value"] * mult
            available = self.mesh.get(resource, 0.0)
            if needed > available:
                deficit = needed - available
                unit_str = req.get("unit", "")
                from .exceptions import TransitionFrozenError
        ...
        for req in requires:
            resource = req["resource"]
            mult = 1.0618 if resource in ("energy", "cycle") else 1.0
            self.mesh[resource] -= req["value"] * mult
